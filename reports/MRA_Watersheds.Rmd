...
lem_comp_df = lem_cap %>%
  ...
  filter(Scenario %in% c("QRF_cap_Points",
                         "MAT+25%")) %>%
  mutate(Scenario = fct_expand(Scenario, "Available Capacity (QRF)"),
...
```{r lem-parr-chnk-fig, fig.cap = "Overall recovery goal of Chinook parr in the Lemhi (MAT +25%) along with available capacity, colored by lower and upper Lemhi."}
lem_bar_p$Chinook.Parr
```

```{r lem-winter-chnk-fig, fig.cap = "Overall recovery goal of Chinook presmolts (overwintering) in the Lemhi (MAT +25%) along with available capacity, colored by lower and upper Lemhi."}
lem_bar_p$Chinook.Presmolts
```

```{r lem-redd-chnk-fig, fig.cap = "Overall recovery goal of Chinook redds in the Lemhi (MAT +25%) along with available capacity, colored by lower and upper Lemhi."}
lem_bar_p$Chinook.Redds
```

```{r lem-parr-sthd-fig, fig.cap = "Overall recovery goal of steelhead parr in the Lemhi (MAT +25%) along with available capacity, colored by lower and upper Lemhi."}
lem_bar_p$Steelhead.Parr
```

```{r lem-winter-sthd-fig, fig.cap = "Overall recovery goal of steelhead presmolts (overwintering) in the Lemhi (MAT +25%) along with available capacity, colored by lower and upper Lemhi."}
lem_bar_p$Steelhead.Presmolts
```

```{r lem-redd-sthd-fig, fig.cap = "Overall recovery goal of steelhead redds in the Lemhi (MAT +25%) along with available capacity, colored by lower and upper Lemhi."}
lem_bar_p$Steelhead.Redds
```

```{r save-plots}
map(names(lem_bar_p), function(.x) {
  ggsave(
    path = "figures/",
