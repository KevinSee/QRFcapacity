...
pts_rch_cor = comp_pop %>%
  pivot_wider(id_cols = c(Species, ESU_DPS:reach_leng),
              names_from = "Method",
              values_from = c("cap_tot", "cap_tot_se")) %>%
...
cap_xy_p = comp_pop %>%
  ...
  pivot_wider(id_cols = c(Species, ESU_DPS:reach_leng, pop_label),
              names_from = "Method",
              values_from = c("cap_tot", "cap_tot_se")) %>%
  filter(cap_tot_Points > 0,
         cap_tot_Reaches > 0) %>%
...
We computed the total capacity of each population using both methods, and compared them. The correlation between the two estimates is shown in Table \@ref(tab:corr-tab). 
...
```{r comp-rel, fig.cap = "Relative difference between the capacity estimates for each population, using the master sample points method as the reference."}
rel_diff_p
```
